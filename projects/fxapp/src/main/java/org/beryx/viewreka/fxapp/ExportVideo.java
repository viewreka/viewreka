...
public class ExportVideo extends GridPane implements FXMLControl {
	...
	public static ExportVideo createWith(GuiSettings guiSettings, ProjectSettings projectSettings, FxChartBuilder<?> chartBuilder, Parameter<?> iteratedParameter, long delay) {
		return new ExportVideo(guiSettings, projectSettings, chartBuilder, iteratedParameter, delay).load();
	}

	private ExportVideo(GuiSettings guiSettings, ProjectSettings projectSettings, FxChartBuilder<?> chartBuilder, Parameter<?> iteratedParameter, long delay) {
		...
		stage.setOnShown(ev -> {
			...
			Thread exportThread = new Thread(() -> {
				...
				if(butAnimatedGif.isSelected()) {
					exportGif(cancelFlag, outputFile, resolution, imageProvider, 0, valueCount-1);
				} else {
					exportMp4(cancelFlag, outputFile, resolution, imageProvider, 0, valueCount-1);
				}
				Platform.runLater(() -> {
					stage.close();
				});
	...
	public void exportGif(AtomicBoolean cancelFlag, File outputFile, VideoResolution resolution, IntFunction<BufferedImage> imageProvider, int startPrmIndex, int endPrmIndex) {
		...
		try(ImageOutputStream output = new FileImageOutputStream(outputFile)) {
			...
			GifSequenceWriter writer = new GifSequenceWriter(output, img.getType(), (int)delay, ckLoop.isSelected());
	...
	public void exportMp4(AtomicBoolean cancelFlag, File outputFile, VideoResolution resolution, IntFunction<BufferedImage> imageProvider, int startPrmIndex, int endPrmIndex) {
		if(endPrmIndex < startPrmIndex) return;
		...
		try {
			...
			mp4Creator.imagesToMP4(cancelFlag, outputFile, i -> imageProvider.apply(startPrmIndex + i), imageCount, delay);
		} catch(IOException e) {
