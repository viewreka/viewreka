...
public abstract class AbstractParameter<T> implements Parameter<T> {
	...
	public String asString(T val) {
		if(val != null && isIterable()) {
			int pos = getPossibleValues().stream().map(v -> v.getValue()).collect(Collectors.toList()).indexOf(val);
			if(pos >= 0) {
	...
	public List<Value<T>> getPossibleValues() {
		...
		synchronized(datasetProvider) {
			if(cachedPossibleValues == null) {
				Dataset dataset = datasetProvider.getDataset();
				List<Value<T>> values = new ArrayList<>();
				for(int i=0; i < dataset.getItemCount(); i++) {
					try {
						T val = (valColumn >= 0) ? dataset.getValue(valColumn, i, type) : dataset.getValue(valColumnName, i, type);
						String textValue = (textValColumn >= 0)
								? dataset.getString(textValColumn, i)
								: (textValColumnName != null)
								? dataset.getString(textValColumnName, i)
								: (val == null) ? "" : val.toString();
						String displayedValue = (displayedValColumn >= 0)
								? dataset.getString(displayedValColumn, i)
								: (displayedValColumnName != null)
								? dataset.getString(displayedValColumnName, i)
								: textValue;
						values.add(new Value<>(val, textValue, displayedValue));
					} catch (Exception e) {
						throw new ViewrekaException("Cannot retrieve the possible value #" + i + " of parameter " + getName(), e);
					}
				}
